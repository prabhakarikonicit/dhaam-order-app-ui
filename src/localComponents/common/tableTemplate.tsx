...
import { TableData, Filter, TableTemplateProps } from "../../types";
...
const TableTemplate = ({
      ...
      if (col.type == "jsx") {
        const isValid = tableData.every((row) => {
          ...
          return (
            ...
            keys.length == 2 &&
            keys[0] == "jsx" &&
            typeof row[col.field][keys[0]] == "object" &&
            keys[1] == "value" &&
            typeof row[col.field][keys[1]] == "string"
          );
        });
        if (!isValid) {
  ...
  useEffect(() => {
    ...
    if (searchValue) {
      result = result.filter((row) => {
        return tableColumns.some((col) => {
          ...
          const value =
            col.type == "jsx" ? row[col.field].value : row[col.field];
    ...
    if (appliedFilter && Object.keys(appliedFilter).length > 0) {
      ...
      const filterType = tableColumns.find(
        (col) => col.field == filterColumn
      )?.type;
      if (filterColumn && filterValue) {
        result = result.filter((item) => {
          const tableCell =
            filterType == "jsx"
              ? item[filterColumn].value
    ...
    if (sortOrder != "none") {
      const sorColType = tableColumns.find(
        (col) => col.field == sortColumn
      )?.type;
      result = [...result].sort((a, b) => {
        let aValue =
          sorColType && sorColType == "jsx"
            ? String(a[sortColumn].value)
            : String(a[sortColumn]);
            ...
            <div className="py-1">
              <button
                className={`block w-full text-left px-4 py-2 text-[12px] font-inter font-[500] text-textHeading ${
                  density === "comfortable" ? "bg-gray-100" : ""
                }`}
                onClick={() => {
